 ...
 */
public class RemoteDbPlugin extends AbstractUIPlugin implements IDatabaseFactoryProvider {

    // The plug-in ID
    public static final String PLUGIN_ID = "eu.hydrologis.jgrass.remotedb"; //$NON-NLS-1$

    public static final String POSTGRESQL = "PostgreSQL"; //$NON-NLS-1$

    // The shared instance
    private static RemoteDbPlugin plugin;
    ...
    public void start( BundleContext context ) throws Exception {
        ...
        if (dbType.equals(POSTGRESQL)) {
            dbPlugin = PostgresqlPlugin.getDefault();
        }
        ...
        if (dbPlugin == null) {
            Display.getDefault().asyncExec(new Runnable(){
                public void run() {
                    ...
                    msgBox
                            .setMessage(dbType
                                    + ": The database plugin for the choosen database could not be loaded. Probably the plugin is not installed.");
                    msgBox.open();
                }
            });
        }
        ...
        String dbHost = store.getString(DbParams.HOST);
        String dbName = store.getString(DbParams.NAME);

        dbPlugin.setConnectionParams(user, passwd, dbName, port, dbHost);
    }
     ...
     */
    public static RemoteDbPlugin getDefault() {
        return plugin;
    }
