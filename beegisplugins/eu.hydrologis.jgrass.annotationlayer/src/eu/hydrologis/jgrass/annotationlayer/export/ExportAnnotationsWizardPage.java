...
public class ExportAnnotationsWizardPage extends WizardPage implements ModifyListener {

    private Text fileText;
    private String filePath = null;

    public ExportAnnotationsWizardPage( String pageName, IStructuredSelection selection ) {
        super(pageName);
        setTitle(pageName);
        setDescription("Export the annotation layer to shapefile."); // NON-NLS-1
        ImageDescriptor imageDescriptorFromPlugin = AbstractUIPlugin.imageDescriptorFromPlugin(AnnotationPlugin.PLUGIN_ID,
                "icons/highlight.gif");
    ...
    public void modifyText( ModifyEvent e ) {
        ...
        if (f.getParentFile().exists() && f.getName().length() > 0) {
